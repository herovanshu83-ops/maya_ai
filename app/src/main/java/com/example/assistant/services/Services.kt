...
class MayaAccessibilityService : AccessibilityService() {
    ...
    private val windowListeners = java.util.concurrent.CopyOnWriteArrayList<(pkg: String, className: String) -> Unit>()

    fun addWindowListener(listener: (pkg: String, className: String) -> Unit) {
        windowListeners.add(listener)
    }

    fun removeWindowListener(listener: (pkg: String, className: String) -> Unit) {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (cls.isNotBlank() && !cls.contains("Layout") && !cls.contains("View")) {
            currentActivity = cls
        }
        if (event.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            windowListeners.forEach { it.invoke(currentPackage, currentActivity) }
        }
    }
    ...
    fun findAndClickByAny(vararg candidates: String): Boolean {
        for (candidate in candidates) {
            if (findAndTapByDescription(candidate)) return true
            if (findAndTapByText(candidate, exact = false)) return true
            if (findAndTapByViewId(candidate)) return true
        }
        return false
    }
    ...
    fun findSearchInputNode(): AccessibilityNodeInfo? {
        ...
        // 1. Check focused input
        val focused = root.findFocus(AccessibilityNodeInfo.FOCUS_INPUT)
        if (focused != null && (focused.isEditable || focused.className?.toString()?.contains("EditText", ignoreCase = true) == true)) {
            return focused
        }

        // 2. Search tree for EditText or search bar
        fun searchInput(node: AccessibilityNodeInfo): AccessibilityNodeInfo? {
            ...
            if (node.isEditable || cls.contains("EditText", ignoreCase = true) ||
                desc.contains("search", ignoreCase = true) || viewId.contains("search", ignoreCase = true) ||
        ...
        return searchInput(root)
    }

    fun findAndClickFirstPlayableOrListItem(): Boolean {
        val root = rootInActiveWindow ?: return false

        fun searchFirstClickable(node: AccessibilityNodeInfo): Boolean {
            val cls = node.className?.toString() ?: ""
            val desc = node.contentDescription?.toString() ?: ""
            val text = node.text?.toString() ?: ""

            // Skip top app bars, search boxes, and tabs
            val isNavOrSearch = desc.contains("search", ignoreCase = true) ||
                    desc.contains("navigate", ignoreCase = true) ||
                    text.contains("search", ignoreCase = true) ||
                    cls.contains("EditText", ignoreCase = true)

            if (!isNavOrSearch && (node.isClickable || desc.isNotBlank() || text.isNotBlank())) {
                val rect = Rect()
                node.getBoundsInScreen(rect)
                // Ensure it's a substantive card/item on the screen below the top bar
                if (rect.width() > 120 && rect.height() > 80 && rect.top > 200) {
            ...
            for (i in 0 until node.childCount) {
                val child = node.getChild(i) ?: continue
                if (searchFirstClickable(child)) return true
            }
            return false
        }

        return searchFirstClickable(root)
    }

    fun detectDialogOrInterruption(): String? {
        val root = rootInActiveWindow ?: return null
        fun searchDialog(node: AccessibilityNodeInfo): String? {
            val cls = node.className?.toString() ?: ""
            if (cls.contains("Dialog", ignoreCase = true) || cls.contains("AlertDialog", ignoreCase = true)) {
                return node.text?.toString() ?: "Dialog"
            }
            for (i in 0 until node.childCount) {
                val child = node.getChild(i) ?: continue
                val res = searchDialog(child)
                if (res != null) return res
            }
            return null
        }
        return searchDialog(root)
    }

    fun dismissDialogIfPossible(): Boolean {
        val dismissLabels = listOf("Not now", "Dismiss", "Close", "Skip", "Cancel", "No thanks", "Got it", "Later")
        for (label in dismissLabels) {
            if (findAndTapByText(label, exact = false) || findAndTapByDescription(label)) {
                return true
            }
        }
        return false
    ...
    fun pressImeSearchOrEnter(): Boolean {
        val root = rootInActiveWindow ?: return false
        val focused = root.findFocus(AccessibilityNodeInfo.FOCUS_INPUT)
        if (focused != null) {
            // Click search action or enter
            if (focused.performAction(AccessibilityNodeInfo.ACTION_CLICK)) return true
        }
        // Fallback: tap the enter/search key position at bottom right of typical soft keyboard
        val dm = resources.displayMetrics
        val kbEnterX = dm.widthPixels * 0.90f
        val kbEnterY = dm.heightPixels * 0.94f
        performTap(kbEnterX, kbEnterY)
        return true
    }
    ...
    private fun clickNodeOrParent(node: AccessibilityNodeInfo): Boolean {
        ...
        // Coordinate Tap fallback using node bounds
        val rect = Rect()
        node.getBoundsInScreen(rect)
    ...
    fun clickVisualTarget(target: com.example.assistant.core.VisualTarget): Boolean {
        // 1. Try finding by viewId
        if (target.viewId.isNotBlank() && findAndTapByViewId(target.viewId)) {
            return true
        }
        // 2. Try finding by contentDescription
        if (target.contentDescription.isNotBlank() && findAndTapByDescription(target.contentDescription)) {
            return true
        }
        // 3. Try finding by text
        if (target.text.isNotBlank() && findAndTapByText(target.text, exact = false)) {
            return true
        }
        // 4. Tap the measured center coordinates
        if (target.bounds.width() > 0 && target.bounds.height() > 0) {
            performTap(target.centerX, target.centerY)
            return true
        }
        return false
    }

    fun typeIntoVisualTarget(target: com.example.assistant.core.VisualTarget, text: String): Boolean {
        // First click to focus
        clickVisualTarget(target)
        // Then set text
